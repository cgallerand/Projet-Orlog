	...
	private static void IADifficileUser2()
	{
		...
		do {
			...
			if (choixia.equalsIgnoreCase("y")) {// le joueur fait le choix de garder seulement certains d�s
				...
				listeDeTotalU1.addAll(listeU2L1C1);// on stocke les d�s gard�s

				listeU2L1Rel1 = choisirDeARelancerIA(listeU2L1, listeU2L1C1);// on appelle la m�thode choisirDeARelancer()
				// pour stocker les d�s � relancer
			...
			if (listeDeTotalU1.size() < 5) {
				...
				if (listeDeTotalU1.size() < 5) {
					...
					String lancer1=saisieString("Appuyez sur Entrer pour lancer les d�s");
